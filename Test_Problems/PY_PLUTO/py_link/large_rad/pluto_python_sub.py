...
def python_input_file(fname,data,cycles=2):
	...
	if data["spectype"]=="brem":
		...
		output.write("BH.rad_type_to_make_wind(bb,models,power,cloudy,brems)   models\n")
...
def pre_calc(ifile,radforce=0):
	...
	for i in range(len(heatcool["rho"])):
		...
		if radforce:
			#The first thing we need to do, it take the r,theta,phi acceleration numners from pluto and turn them into cartesian...
			g_r=(D.gr[heatcool["i"][i]][heatcool["j"][i]]*UNIT_ACCELERATION)
			g_t=(D.gt[heatcool["i"][i]][heatcool["j"][i]]*UNIT_ACCELERATION)
			g_p=(D.gp[heatcool["i"][i]][heatcool["j"][i]]*UNIT_ACCELERATION) #We dont do anything with this at the moment
			
			theta=heatcool["thetacen"][i]
			
			

			
			g_x=(g_r*np.sin(theta)+g_t*np.cos(theta))
			g_y=g_p                                      #We also dont do anything with thus.
			g_z=(g_r*np.cos(theta)-g_t*np.sin(theta))
			
			if i==itest:
				print g_r,g_t,g_p,g_x,g_y,g_z,theta
						
			ideal_prefactor=(heatcool["rad_f_w"][i]/heatcool["rho"][i]/heatcool["vol"][i])/g_x
			change=ideal_prefactor/D.gx_pre[heatcool["i"][i]][heatcool["j"][i]]
			if change<max_accel_change:
				change=max_accel_change
			elif change>(1./max_accel_change):
				change=(1./max_accel_change)
			gx_pre.append(change*D.gx_pre[heatcool["i"][i]][heatcool["j"][i]])
			
			gy_pre.append(1.0)
			
			ideal_prefactor=(heatcool["rad_f_z"][i]/heatcool["rho"][i]/heatcool["vol"][i])/g_z
			change=ideal_prefactor/D.gz_pre[heatcool["i"][i]][heatcool["j"][i]]
			if change<max_accel_change:
				change=max_accel_change
			elif change>(1./max_accel_change):
				change=(1./max_accel_change)
			gz_pre.append(change*D.gz_pre[heatcool["i"][i]][heatcool["j"][i]])	
		else:		
			gx_pre.append(1.0)
			gy_pre.append(1.0)
			gz_pre.append(1.0)
	...
	titles=titles+["gx_pre","gy_pre","gz_pre"]	
