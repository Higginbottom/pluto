...
data["python_ver"]="py84"  #the version of python to use
...
time={}
time["t0"]=10.0  #The run time for the initial pluto run - the first run is to produce a starting geometry
time["dt"]=-1   #The time between calls to pluto if negative then we will be raming up the timescale
time["dt_min"]=10.
time["dt_max"]=1000.
time["dt_rampup"]=100.
...
data["T_x"]=400000.
data["CENT_MASS"]=0.8*c.M_sun.cgs.value
data["MU"]=0.6
data["XI_cool_max"]=15.5
...
data["R_0"]=1e9       #THe radius we are using to set the density
data["RHO_0"]=1e-10 
...
#    print("Last dbl file was for time",last_dbl_time)
    ...
    request= input("Are you sure (Y/N)?")
    if request=="Y":
        try:
    ...
    if py_cycles>py_last_completed:
        ...
        flag=1
        time["t0"]=time=["t0"]-time["dt"]
        
    else:
        print ("We will be starting off with a pluto run")
        print(("Last dbl time=",last_dbl_time,"Default starting t0=",time["t0"]+float(istart)*time["dt"]))
...
pps.loop(time,istart,py_cycles,data,flag)
