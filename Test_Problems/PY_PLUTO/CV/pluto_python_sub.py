...
def python_input_file(fname,data,cycles=2):
    ...
    output.write("Central_object.radius(cm)                  6.9e+08\n")
    output.write("\n")
    ...
    output.write("Boundary_layer.luminosity(ergs/s) 5e34\n")
    output.write("Boundary_layer.temp(K) 100000\n")    
...
def loop(time_dict,istart,py_cycles,data,flag):
    
    
    if time_dict["dt"]>0:
        dt=time_dict["dt"] 
    else:
        dt=time_dict["dt_max"]-time_dict["dt_min"]
    ...
    for i in range(istart,200):  
        ...
        if flag==0: #If flag is not set - then we run pluto first 
            if i==0: #We are on our first cycle
                ...
                last_dbl_time=time
                 ...
                 proc=subprocess.Popen(cmdline,shell=True,stdout=subprocess.PIPE)
                 last_dbl_time=float(proc.stdout.read().split()[1])
                 dt=np.min([time_dict["t0"]+((time_dict["dt_max"]-time_dict["dt_min"])/time_dict["dt_rampup"])*i,time_dict["dt_max"]])
        ...
        out.write("Running python with "+str(py_cycles)+" cycles\n") 
        print("Running python with "+str(py_cycles)+" cycles\n") 
        out.write(cmdline+"\n")
        print((cmdline+"\n"))
