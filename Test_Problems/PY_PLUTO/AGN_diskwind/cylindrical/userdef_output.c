...
double ***dvr_dr,***dvz_dz,Vc[NVAR],g[3];
double t,v_th,M_max;



double rho,p,***T,xi,ne,nH,n,mu,lx,tx,r,***gr_out,***gz_out,ciso;
double *x1   = grid->x[IDIR],  *x2  = grid->x[JDIR],  *x3  = grid->x[KDIR];


dvr_dr = GetUserVar("dvr_dr");
dvz_dz = GetUserVar("dvz_dz");

//M = GetUserVar("M");
gr_out= GetUserVar("g_r");
gz_out= GetUserVar("g_z");
...
//      if (M[k][j][i]>M_max) M[k][j][i]=M_max;
      ...
      dvr_dr[k][j][i]=dvr_dr_array[k][j][i];
      dvz_dz[k][j][i]=dvz_dz_array[k][j][i];
