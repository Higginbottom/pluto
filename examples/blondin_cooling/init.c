 ...
 *********************************************************************** */
{
	double temp,cisosqrd,cent_mass,rho_alpha,disk_mdot,rho_0,r_0,mu,tx;
	
	cent_mass=g_inputParam[CENT_MASS];  //Central mass
	rho_alpha=g_inputParam[RHO_ALPHA];  //Dropt off exponent of density
	disk_mdot=g_inputParam[DISK_MDOT];  //disk accretuion rate - needed for the temperature
...
//we work out the temperaure in an accretion disk at r=x1	
  ...
  temp=pow(((3.0*cent_mass*disk_mdot*CONST_G)/(8.0*CONST_PI*CONST_sigma*pow((x1*sin(x2)),3.0))),0.25);
...
//we need the isothermal sound speed to work out the hydrostatic disk structure 
  ...
  v[RHO]=rho_0*pow((x1/r_0),-1.0*rho_alpha); //midplane density at this radius
  v[RHO]=v[RHO]*exp(-1.0*CONST_G*cent_mass*pow((x1*cos(x2)),2)/(cisosqrd*2.0*pow(x1,3))); //hydrostatic strucuture
...
cisosqrd=g_gamma*temp/(KELVIN*mu);
  ...
  v[VX1] = 0.0;
  v[VX2] = 0.0;
  v[VX3]=sqrt((CONST_G*cent_mass*sin(x2)*sin(x2))/x1);
