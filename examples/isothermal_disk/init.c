 ...
 *********************************************************************** */
{
	double temp,cisosqrd,cent_mass,rho_alpha,disk_mdot,rho_0,r_0;
	...
	g_isoSoundSpeed = g_inputParam[CISO];
	
	

  v[RHO]=rho_0*pow((x1/r_0),-1.0*rho_alpha);
  
  
  v[RHO]=v[RHO]*exp(-1.0*1.0*pow((x1*cos(x2)),2)/(g_isoSoundSpeed*g_isoSoundSpeed*2.0*pow(x1,3)));
  ...
  v[VX1] = 0.0;
  v[VX2] = 0.0;
  v[VX3]=sqrt((1.0*sin(x2)*sin(x2))/x1);

  #if HAVE_ENERGY
  v[PRS] = g_isoSoundSpeed*g_iso*SoundSpeed*v[RHO]/g_gamma;
  #endif
  v[TRC] = 0.0;
...
rho_alpha=g_inputParam[RHO_ALPHA];
  ...
  {    /* -- check solution inside domain -- */
	TOT_LOOP(k,j,i)
	{
		if (j==grid[JDIR].np_int_glob+1 && i>1 && i<grid[IDIR].np_int_glob+2)  //This should be the last 'real' theta bin - before the ghost zones.
		{
			...
			d->Vc[VX1][k][j][i]=0.0;
		    d->Vc[VX3][k][j][i]=sqrt((1.0*sin(x2[j])*sin(x2[j]))/x1[i]);
			
		}
		if (d->Vc[RHO][k][j][i] < 1.e-24) 
		{
	         d->Vc[RHO][k][j][i] = 1.e-24;
		 } 
	 }	
  }
  ...
  if (side == X2_END){  /* -- X2_END boundary -This is the midplane- */
    if (box->vpos == CENTER) {
      BOX_LOOP(box,k,j,i){printf("test %i %i %i %e\n",i,j,k,d->Vc[RHO][k][j][i]);  }
    }else if (box->vpos == X1FACE){
      BOX_LOOP(box,k,j,i){  }
    }else if (box->vpos == X2FACE){
      BOX_LOOP(box,k,j,i){  }
...
#elif GEOMETRY == SPHERICAL return 
	-1.0*cent_mass*CONST_G/x1;
#endif}
#endif
